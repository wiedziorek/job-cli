...
class ShotgunReader(ShotgunPlugin, PluginManager):
    ...
    def read_asset(self, ha_project, ha_type, ha_asset, fields=None):
        ...
        :param ha_project:  Name of the project
        :param ha_type:     Name of the sequence or asset type
        :param ha_asset:    Name of the shot or asset
        ...
        if self.get_sg_type(ha_type) == "Shot":
            sg_sequence = self.get_sg_sequence(sg_project, ha_type)
            return self.get_sg_shot(sg_project, sg_sequence, ha_asset, fields)
        else:
            sg_asset_type = self.get_sg_asset_type(ha_type)       
            return self.get_sg_asset(sg_project, sg_asset_type, ha_asset, fields)

    def read_type(self, ha_project, ha_type, fields=None):
    ...
    def read_project_assets(self, ha_project, fields=None):
    ...
    def read_project_shots(self, ha_project, fields=None):
    ...
    def read_project(self, ha_project, fields=None):
        ...
        :returns:           A dictionary with information on shots
        """
        assets = self.read_project_assets(ha_project, fields)
        shots = self.read_project_shots(ha_project, fields)
        return assets + shots

    def get_assets_by_user(self, ha_project, user, fields=None):
        ...
        sg_assets = self.read_project_assets(ha_project, fields)
        result = list()
    ...
    def get_shots_by_user(self, ha_project, user, fields=None):
        ...
        fields.append('tasks')
        sg_user = self.get_sg_user(user)
        sg_shots = self.read_project_shots(ha_project, fields)
        result = list()
    ...
    def __call__(self, options):
        if options["--field-list"]:
            return self.read_field_list(options["PROJECT"],
                                        options["TYPE"],
                                        options["ASSET"])
        elif options["ASSET"]:
